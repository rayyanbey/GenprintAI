...
function SplashCursor({
  // Optimized default values for better performance
  SIM_RESOLUTION = 64, // Reduced from 128
  DYE_RESOLUTION = 512, // Reduced from 1440
  CAPTURE_RESOLUTION = 512,
  DENSITY_DISSIPATION = 4, // Increased for faster fade
  ...
  useEffect(() => {
    ...
    let idleFrames = 0;
    const MAX_IDLE_FRAMES = 180; // Stop after ~3 seconds of no activity
    ...
    function updateFrame() {
      ...
      const hadActivity = applyInputs();
      
      // Reset idle counter if there was activity
      if (hadActivity) {
        idleFrames = 0;
      } else {
        idleFrames++;
      ...
      // Only continue animation if there's recent activity
